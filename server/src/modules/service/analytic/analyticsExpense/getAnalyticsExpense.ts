...
export const getAnalyticsExpense = async (user: User, input: AnalyticsInput): Promise<AnalyticsCategory[]> => {
  ...
  if (shouldApplyDateFilters != null) {
    ...
    if (gteFilter !== null && lteFilter !== null) {
      ...
      analyticsCategory = await queryAnalyticsExpense(userId, gte, lte)
    }
  }
